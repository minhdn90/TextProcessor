...
public class TextProController {
	...
	//private final String quotations = "([\"])(?:(?=(\\?))\2.)*?\1";
	...
	@FXML
	private void handleHightlightPDF(){
		try{
			if (!spellingBox.isSelected() && !autocompleteBox.isSelected()){
				Alert alert = new Alert(AlertType.ERROR);
				alert.setTitle("Àu Àu");
				alert.setHeaderText(null);
				alert.setContentText("Cún yêu quên tick checkboxes oy");
				alert.showAndWait();
			}
			else if (linkingWords == null && spellingBox.isSelected()){
				StringBuilder regexBuilder = new StringBuilder ("");
				File f = new File("data\\transitionwords.txt");
				FileInputStream fis = new FileInputStream(f);
				Scanner in = new Scanner(fis);
	
				while(in.hasNextLine())
					regexBuilder.append("[^a-zA-Z]" + in.nextLine() + "[^a-zA-Z]" + "|");
				in.close();
				linkingWords = regexBuilder.substring(0, regexBuilder.length()-1);
			}
		}
		catch(Exception e){}
		try{
			//if (spellingBox.isSelected())
			//	mainApp.getAnnotator().highlight(mainApp.getPDFDoc(), linkingWords);
			if (autocompleteBox.isSelected())
				mainApp.getAnnotator().highlight(mainApp.getPDFDoc(), quotations);
			mainApp.getPDFDoc().save(mainApp.getOutputFilePath());
		}catch(Exception e){}
	}
	
	@FXML
	private void handleHiliTransitions(){
	}
	
	@FXML
	private void handleHiliQuotations(){
	}
}
